...
Universal Multilingual stores one canonical object per content item and applies language overlays at render time. Version 1.20.0 makes "Approve page" / "Approve all ready languages" also publish that page's prepared localized URL for each approved language, so a bulk translate-then-approve run across many pages and languages never leaves a manual "Publish URL" click behind; a missing slug candidate or a collision is reported, never silently blocking the approval. Version 1.19.0 fixes a redirect_canonical() self-loop on bare non-default language home URLs (#64) and adds an authenticated preferred-language redirect: a logged-in visitor landing on an unprefixed URL is sent once to their stored Regional Preferences language, without changing anonymous URL-authoritative resolution. Version 1.18.0 makes the translation workflow scale to many pages by many languages: load one page with every target language as a tab, translate all selected languages with one action, a Review Queue card that shows every language for a page together (a forgotten language is never hidden), an "Approve all ready languages" action, and an N-pages-by-M-languages Site Translate matrix. It never changes a language's publication status; writing into an already-published language needs an explicit acknowledgement. No schema change. Version 1.17.0 makes the Review Queue object-level: pending translations are grouped by page/product and language with human field labels, and one "Approve page" action approves every eligible pending field at once while spelling out anything still missing. Version 1.16.0 makes the localized URL feel like part of translation (it is proposed automatically, the technical route controls move under Advanced), adds a "Submit selected for review" bulk action, and translates WooCommerce product short descriptions on the storefront. Version 1.15.0 surfaces AI translation as one obvious action: a "Translate with AI" control on the Translator Workspace and a "Translate selected with AI" bulk action, both driven by the existing background Jobs pipeline, with manual, reviewed and in-review translations never overwritten. Version 1.13.0 rebuilds "Add a language" around selection: the URL code, locale, name, native name and direction are derived server-side from a bundled offline locale registry. Version 1.12.0 adds logged-in Regional Preferences (preferred language) and an optional floating language selector (default off). Version 1.11.0 adds Site Translate (coverage-aware picker, chunked Jobs, Run batch now, Localized URL batch). Version 1.10.0 adds the DeepSeek AI provider and per-provider generation settings. Version 1.9.0 rebranded to Universal Multilingual.
...
= 1.20.0 =
* "Approve page" and "Approve all ready languages" now also publish that page's prepared localized URL for each language they approve, best-effort: a missing slug candidate, an unpublished language or a URL collision is reported in the response, never a failed approval. Removes the separate manual "Publish URL" click that a bulk translate-then-approve run across many pages x languages otherwise required for every single one.
...
* New authenticated preferred-language redirect: a logged-in visitor landing on an unprefixed (default-language) URL is redirected once to their stored Regional Preferences language. Anonymous resolution stays strictly URL-authoritative (ADR-0024) and an already-prefixed URL is never overridden.
* No database migration. Schema stays at version 10; settings shape stays at 3.
