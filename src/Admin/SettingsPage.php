 ...
 * `admin-post.php` handlers for the localized-URL activation controls.
 * Milestone 1 ships no REST API, because nothing here needs one — a REST layer
 * would exist only to be called by JavaScript this milestone does not ship
 * (ADR-0002 scope note). The segment editor in the next milestone is what
 * actually requires incremental per-segment saves, and REST arrives with it.
 *
 * The Languages list/add/edit screen and its create/update/delete handlers
 * live in {@see LanguagesScreen}; this class owns the menu it hangs from.
 *
...
final class SettingsPage {
	...
	/**
	 * The Languages list/add/edit screen (owns its own form handlers).
	 *
	 * @var LanguagesScreen
	 */
	private LanguagesScreen $languages_screen;
	...
	public function __construct(
		...
		$this->languages_screen  = new LanguagesScreen( $languages );
	...
	public function add_menus(): void {
		add_menu_page(
			...
			'manage_options',
			self::MENU_SLUG,
			array( $this->languages_screen, 'render' ),
